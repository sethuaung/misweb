...
//    }
    ...
    public function addButtonCustom()
    {
        ...
        if(companyReportPart() == "company.moeyan"){
            ...
            <a href="' . url("/admin/history_pop?loanClosed=1&loan_id={$this->id}") . '" data-remote="false" data-toggle="modal" data-target="#show-detail-modal" class="btn btn-xs btn-info"><i class="fa fa-user"></i></a> 
            ';
        }else{
            return $b . '

            <a  class="btn btn-xs btn-danger" onclick="return confirm(\'Are you sure?\')" href="' . url("/admin/cancel-activated?id={$this->id}") . '"  ><i class="fa fa-minus-user"></i> Cancel</a>
            <a href="' . url("/admin/print_schedule?loan_id={$this->id}") . '" data-remote="false" data-toggle="modal" data-target="#show-detail-modal" class="btn btn-xs btn-info"><i class="fa fa-trash"></i></a>
            <a href="' . url("/admin/payment_pop?loan_id={$this->id}") . '" data-remote="false" data-toggle="modal" data-target="#show-detail-modal" class="btn btn-xs btn-info"><i class="fa fa-edit"></i></a>
            ';
        }
    ...
    public static function boot()
    {
        ...
        static::addGlobalScope('disbursement_status', function (Builder $builder) {
            $builder->where('disbursement_status', 'Closed');
        });
    }
