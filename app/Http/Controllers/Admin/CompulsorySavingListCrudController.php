...
class CompulsorySavingListCrudController extends CrudController
{
    public function setup()
    {
        ...
        */
        ...
        $this->crud->setEntityNameStrings('Compulsory List', 'Compulsory List');
       ...
       //$this->crud->setFromDb();
        ...
        if(companyReportPart() != 'company.mkt')
        {
            ...
            false,
                function($value) { // if the filter is active
                    $this->crud->addClause('join', getLoanTable(), 'loan_compulsory.loan_id', getLoanTable().'.id');
...
//            }
    ...
    {
        $rows = Loan2::selectRaw(getLoanTable().'.id, branches.title')
            ->leftJoin('branches','branches.id','=',getLoanTable().'.branch_id')->get();
        return $rows;
