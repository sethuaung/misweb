    ...
    public function setup()
    {
        ...
        */
        ...
        if(CompanyReportPart() == 'company.mkt'){
            $this->crud->addClause('where','clients.customer_group_id','!=',11);
        ...
        if (companyReportPart() == 'company.moeyan'){
            $this->crud->addFilter([ // simple filter
                'type' => 'dropdown',
                'name' => 'loan_completed_customer',
                'label'=> _t("Completed Loan"),
            ], [
                1 => 'Loan Completed Customer',
            ],
                function($value) {
                    $loan = \App\Models\Loan::where('id' ,'>' ,0)->where('disbursement_status','closed')->pluck('client_id')->toArray();
                    $client_ids = array();
                    $first_result = array();
                    $second_result = array();
                    $final_result = array();
                    $client_ids = array_unique($loan);
                    foreach($client_ids as $client_id){
                        $loan_closeds = \App\Models\Loan::where('client_id',$client_id)->get()->toArray();
                        foreach($loan_closeds as $loan_closed){
                            if($loan_closed['disbursement_status'] != 'Activated' && $loan_closed['disbursement_status'] != 'Canceled' && $loan_closed['disbursement_status'] != 'Approved' && $loan_closed['disbursement_status'] != 'Declined'){
                                array_push($first_result,$loan_closed['client_id']);
                            }
                            else{
                                //dd($loan_closed);
                                array_push($second_result,$loan_closed['client_id']);
                            }
                        }
                    }
                    $final_result = array_diff($first_result,$second_result);
                    $client = array_unique($final_result);
                    //dd($client);
                    $this->crud->addClause('whereIn', 'id', $client);
                }
            );
        }
        $this->crud->addColumn([
        ...
        $this->crud->addField([
            ...
            'default' => optional($m)->branch_id??session('s_branch_id'),
            'name' => 'branch_id', // the column that contains the ID of that connected entity
            'entity' => 'branch_name', // the method that defines the relationship in your Model
            'attribute' => "title", // foreign key attribute that is shown to user
            'model' => "App\\Models\\Branch", // foreign key model
...
//            'location_group' => 'General',
        ...
        if (companyReportPart() != 'company.bolika'){
            $this->crud->addField([
                ...
                'tab' => _t('personal_detail'),
//            'location_group' => 'General',
                'default' => optional($m)->center_leader_id-0
            ]);
        ...
        if (companyReportPart() == 'company.bolika'){
            ...
            $this->crud->addField([
                'name' => 'nrc_number_new',
                'label' => _t('nrc_number1 *'),
                'type' => 'text',
                'default' => optional($m)->nrc_number,
                'attributes' => [
                    'id' => 'nrc_new',
                ],
                'wrapperAttributes' => [
...
//            'location_group' => 'General',
        ...
        $this->crud->addField([
            ...
            'label' => _t('dob'),
            'type' => 'date2',
            'default' => optional($m)->dob??'1999-01-01',
            'wrapperAttributes' => [
                'class' => 'form-group col-md-3'
            ],
            'tab' => _t('personal_detail'),
        ...
        if (companyReportPart() != 'company.bolika') {
            $this->crud->addField([
                ...
                'allows_null' => false,
                'default' => optional($m)->education??1,
                'wrapperAttributes' => [
                    'class' => 'form-group col-md-3'
                ],
                'tab' => _t('personal_detail'),
...
//            'default' => 1,
            ...
            'default' => optional($m)->marital_status,
        ]);
        ...
        $this->crud->addField([
            ...
            'tab' => _t('family_information'),
            'default' => optional($m)->father_name,
        ]);
        $this->crud->addField([
            'name' => 'husband_name',
            ...
            'tab' => _t('family_information'),
            'default' => optional($m)->husband_name,
        ]);
        $this->crud->addField([
            ...
            'tab' => _t('family_information'),
            'default' => optional($m)->occupation_of_husband,
...
//                'type' => 'enum',
                ...
                'default' => optional($m)->you_are_a_group_leader
            ]);
...
//                'type' => 'enum',
                ...
                'tab' => _t('personal_detail'),
                'options' => ['Yes' => 'Yes', 'No' => 'No'],
                'default' => optional($m)->you_are_a_center_leader
            ]);
        }
        ...
        $this->crud->addField([
            ...
            'name' => 'register_date',
            'type' => 'date_picker',
            'default' => Carbon::parse(optional($m)->register_date)->format('Y-m-d')??date('Y-m-d'),
            'date_picker_options' => [
                'format' => 'yyyy-mm-dd',
            ],
            'script' => 'change',
...
//            'location_group' => 'General',
        ...
        $this->crud->addField([
            ...
            'tab' => _t('personal_detail'),
            'location_group' => 'General',
            'default' => optional($m)->customer_group_id
        ]);
        ...
        $this->crud->addField([
            ...
            'tab' => _t('personal_detail'),
//            'location_group' => 'General',
            'default' => optional($m)->loan_officer_id
        ]);
        ...
        if(getEnvCountry() == 'KH') {
            ...
            $this->crud->addField([
                ...
                'state_default' => optional($m)->province_id,
                'district_default' => optional($m)->district_id,
                'township_default' => optional($m)->commune_id,
                'village_default' => optional($m)->village_id,
                'ward_default' => optional($m)->ward_id,
                'house_number_default' => optional($m)->house_number,
                'address_name_default' => optional($m)->address2,
...
//                'disk' => 'uploads' // if you store files in the /public folder, please ommit this; if you store them in /storage or S3, please specify it;
            ...
            $this->crud->addField([
                ...
                'type' => 'image2',
                'upload' => true,
                'default' => \optional($m)->photo_of_client??'No_Image_Available.jpg',
...
//        ]);
        ...
        if (companyReportPart() == 'company.moeyan') {
            $this->crud->addColumn([
                ...
                'type' => 'closure',
                'function' => function($entry) {
                    if ($entry->condition=="Have Loan"){
                        return '<button class="btn btn-success btn-xs" >'.$entry->condition.'</button>';
                    }
                    elseif ($entry->condition=="Black Listed"){
                        return '<button class="btn btn-xs" style="background-color:#333;color:white;">'.$entry->condition.'</button>';
                    }
...
//       dd($request->all());
        ...
        if($m != null){
            $m->status = 'completed';
            if ($m->save()){
                /* $c = ClientU::find( $this->crud->entry->id);
                 $c->photo_of_client = $m->photo_client;
                 $c->nrc_photo = '["'.$m->nrc_photo.'"]';

                 if($c->save()){
                     return redirect('admin/approve-client-pending');
                 }*/

                ClientU::where('id',$this->crud->entry->id)
                    ->update([
                        'photo_of_client' => $m->photo_client,
                        'nrc_photo' => '["'.$m->nrc_photo.'"]'
                    ]);
