...
use App\Models\{GeneralJournal, Product, UserU, PaymentHistory};
 ...
 */
class LoanItemRepaymentsReportController extends CrudController
{
    public function setup()
    {
        ...
        |--------------------------------------------------------------------------
        */
        $this->crud->setModel('App\Models\LoanItemPayment');
        $this->crud->setRoute(config('backpack.base.route_prefix') . '/loan-item-repayment');
        $this->crud->setEntityNameStrings('Loan Item Repayment', 'Loan Item Repayments');
        ...
        */
        ...
        if(companyReportPart() == "company.moeyan"){
            ...
            function($value) { // if the filter is active
                $this->crud->addClause('whereHas', 'general_jour', function($query) use($value) {
...
//        ]);
        ...
        $this->crud->addColumn([
            ...
            'orderable' => true,
            'function' => function($entry) {
                $production_id =  optional($entry->loan_disbursement)->product_id;
        ...
        $this->crud->addColumn([
            ...
            'function' => function($entry) {
                if(companyReportPart() == "company.moeyan"){
                    $gen_jour = \App\Models\GeneralJournal::where('reference_no', $entry->payment_number)->first();
                    $branch = \App\Models\BranchU::find(optional($gen_jour)->branch_id);
