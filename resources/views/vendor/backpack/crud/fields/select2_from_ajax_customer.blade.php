    ...
    <select
        ...
        @if ($old_value)
            ...
            <option value="{{ $item->getKey() }}" selected>
                {{ (optional($item)->code != null? optional($item)->code . '-' : '') . $item->{$field['attribute']} }}
            </option>
            @endif
        @endif
    </select>
        @if(isset($field['suffix'])) <div class="input-group-addon">
            <a href="" data-remote="false" data-toggle="modal" data-target="#show-create-customer"><span class="glyphicon glyphicon-plus"></span></a>
        </div> @endif
...
@if ($crud->checkIfFieldIsFirstOfItsType($field, $fields))
    ...
    <div class="modal fade" id="show-create-customer" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg modal-dialog-x">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title" id="myModalLabel">{{_t('Customer')}}</h4>
                </div>
                        ...
                        processResults: function (data, params) {
                            ...
                            var result = {
                                results: $.map(data.data, function (item) {
                                    ...
                                    return {
                                        text: (item['code'] != null? item['code'] + '-' :'') + item[textField],
...
<script>
    $("#show-create-customer").on("show.bs.modal", function(e) {
        var link = '{{url('admin/customer/create?is_frame=1')}}';
