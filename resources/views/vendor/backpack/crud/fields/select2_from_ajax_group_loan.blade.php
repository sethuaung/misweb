    ...
    <select
        ...
        @if ($old_value)
            ...
            <option value="{{ $item->getKey() }}" selected>
                {{ $item->{$field['attribute']}."-".$item->{$field['attribute1']} }}
            </option>
            @endif
        @endif
    </select>
...
<div class="modal fade" id="show-create-group" {{--tabindex="-1"--}} role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-x">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title" id="myModalLabel">{{_t('Group')}}</h4>
            </div>
...
<script>
    ...
    <script>
        $("#show-create-group").on("show.bs.modal", function(e) {
            ...
            var link = '{{url('admin/grouploan/create?is_frame=1')}}';
