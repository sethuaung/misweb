...
<div class="form-group col-md-12 checklist_dependency"  data-entity ="{{ $field['field_unique_name'] }}" @include('crud::inc.field_wrapper_attributes')>
    ...
    <div class="row" >
        ...
        <div class="col-xs-12"  style="background-color: #f4f4f5;">
    ...
    <div class="row">
        ...
        @php
            $m  = $secondary_dependency['model']::all()
        @endphp
        @if($m != null)
            @php
            ...
            @foreach ($gg as $g => $mg)
                ...
                <div class="col-xs-12"><h5 style="color: red;">{{ ucfirst($g) }}</h5></div>
          ...
          thisField.find('.primary_list').change(function(){
            ...
            if($(this).is(':checked')){
              ...
              $.each(dependencyJson[idCurrent], function(key, value){
                //check and disable secondies checkbox
                thisField.find('input.secondary_list[value="'+value+'"]').prop( "checked", true );
                thisField.find('input.secondary_list').prop( "disabled", true );
                //remove hidden fields with secondary dependency if was setted
                var hidden = thisField.find('input.secondary_hidden[value="'+value+'"]');
                if(hidden)
                  hidden.remove();
              ...
              $.each(secondary, function(index, secondaryItem){
                ...
                if(ok){
                  thisField.find('input.secondary_list[value="'+secondaryItem+'"]').prop('checked', false);
                  thisField.find('input.secondary_list').prop('disabled', false);
                }
              });
