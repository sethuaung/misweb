    ...
    <table class="table table-bordered table-striped table-condensed">
        <thead>
            <tr style="background-color:#3c8dbc;color:white;">
                ...
                <th class="text-center">{{_t('debit')}}</th>
                <th class="text-center">{{_t('credit')}}</th>
            ...
            @if($journal_details != null)
                @if(count($journal_details) > 0)
                    @foreach($journal_details as $journal_detail)
                        @include('partials.account.add-detail',['rd' => $journal_detail])
                    @endforeach
                @endif
            @endif
        ...
        <tfoot>
            <tr>
                <th colspan="2" class="text-right">{{_t('total')}}</th>
                <th class="text-right"><span class="total_dr">0</span><span class="currency2"> </span></th>
    ...
    <script>
        ...
        $(function () {
           ...
           $('[name="acc_chart_id"]').on('change', function () {
              ...
              if (acc_chart_id > 0) {
                  $.ajax({
                      url:'{{url('admin/general-journal-add-detail')}}',
                      data:{
