...
{{--                        <a href="#" class="btn btn-dark" type="button" id="excel"><i class="fa fa-download" aria-hidden="true"></i> Excel</a>--}}
                ...
                <div class="overflow-hidden">
                    ...
                    <tbody>
                        ...
                        @if(in_array('com-saving-accrued-interests', $Url))
                            <tr style="background-color:#ddd;">
                                ...
                                <td class="text-bold" id="interest_amount_total"></td>
                                <td class="text-bold" id="accrued_interest_total"></td>
                                <td class="text-bold" id="deposit_amount_total"></td>
                                <td class="text-bold" id="saving_principle_total"></td>
                            </tr>
                        @endif
        ...
        @if(companyReportPart() == 'company.mkt')
            ...
            @endphp
            @if (in_array('com-saving-accrued-interests', $Url))
            {{--<form class="form-inline" method="GET" action="{{route('saving-accrued-interest.excel')}}">
            ...
            @endif
            @if (in_array('saving-withdrawals', $Url))
            <form class="form-inline" method="GET" action="{{route('saving-withdrawal.excel')}}">
                <div class="form-group mx-sm-3" style="margin-right: 4px;">
                    <select class="form-control" id="branch_id" name="branch_id">
                        @foreach ($select_branches as $select_branch)
                            <option value="{{$select_branch->id}}">{{ $select_branch->title}}</option>
...
//        $.ajax({
//         url: "{{route('saving-accrued-interest.excel')}}",
//         method: "get",
//            data: {
//                branch_id:branch_id
//            },
...
jQuery(document).ready(function($) {
    ...
    //     var total_interest_amount = crud.table.column(10).data().sum();
    //     var total_accrued_interest = crud.table.column(11).data().sum();
    //     var total_deposit_amount = crud.table.column(12).data().sum();
    //     var total_saving_principle = crud.table.column(13).data().sum();

    //   $("#interest_amount_total").html(total_interest_amount.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,'));
    //   $("#accrued_interest_total").html(total_accrued_interest.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,'));
    //   $("#deposit_amount_total").html(total_deposit_amount.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,'));
    //   $("#saving_principle_total").html(total_saving_principle.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,'));
...
//         'payment_ref': $('#text-filter-payment-ref').val(),
//         'search': crud.table.search(),
//     }

//     var url = "{{URL::to('/admin/report/saving-deposit/excel')}}?" + $.param(query)
//     window.location = url;
// });
</script>
@endsection
