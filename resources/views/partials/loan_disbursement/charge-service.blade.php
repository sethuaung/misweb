...
@if(isset($id))
    <?php
    $id = $id!=null?$id:0;
    $loan_charge = \App\Models\LoanCharge::where('loan_id',$id)->get();
    $loan_compulsory = \App\Models\LoanCompulsory::where('loan_id',$id)->first();
    ?>
    @if($loan_charge != null)
        <h3>Charge</h3>
        <table class="table table-bordered" id="table-data" style="margin-top: 20px;">
            <thead>
            <tr>
                <th>Service Name</th>
            ...
            <tbody>
            @foreach($loan_charge as $c)
                <?php
                $rand_id = rand(1,1000).time().rand(1,1000);
                ?>
                <tr>
                    <td>
                        <input type="hidden" class="form-control" name="charge_id[{{$rand_id}}]" value="{{optional($c)->charge_id}}">
...
//                            dd($status);
    ...
    @if($loan_compulsory!=null)
        ...
        <table class="table table-bordered" id="table-data" style="margin-top: 20px;">
            ...
            <tr>
                <td>
                    <input type="hidden" class="form-control" name="compulsory_id" value="{{optional($loan_compulsory)->compulsory_id}}">
                    <input type="hidden" class="form-control" name="compound_interest" value="{{optional($loan_compulsory)->compound_interest}}">
                    <input type="hidden" class="form-control" name="override_cycle" value="{{optional($loan_compulsory)->override_cycle}}">
                    <input type="hidden" class="form-control" name="loan_compulsory_id" value="{{optional($loan_compulsory)->id}}">
                    <input class="form-control" name="product_name" value="{{optional($loan_compulsory)->product_name}}">
                </td>
                <td>
                    <input class="form-control" name="saving_amount" value="{{optional($loan_compulsory)->saving_amount}}">

                </td>
                <td>
                    <select class="form-control" name="c_charge_option">
                        @foreach($charge_option as $ke => $va)
                            <option value="{{$ke}}" {{optional($loan_compulsory)->charge_option==$ke?'selected="selected"' : ''}}>{{$va}}</option>
                        @endforeach
                    </select>

                </td>
                <td>
                    <input class="form-control" name="c_interest_rate" value="{{optional($loan_compulsory)->interest_rate}}">
                </td>
                <td>
                    <select class="form-control" name="compulsory_product_type_id">
                        @foreach($compulsory_product_type as $ke => $va)
                            <option value="{{$ke}}" {{optional($loan_compulsory)->compulsory_product_type_id==$ke?'selected="selected"' : ''}}>{{$va}}</option>
                        @endforeach
                    </select>
                ...
                <td>
                    <select class="form-control" name="c_status">
                        @foreach($status as $ke => $va)
                            ...
                            <option value="{{$ke}}" {{optional($loan_compulsory)->status==$ke?'selected="selected"' : ''}}>{{$va}}</option>
                        @endforeach
                    </select>
