...
@section('after_scripts')
    ...
    <script>
        jQuery(document).ready(function($) {
            ...
            crud.table.on('draw.dt', function () {
              $("#principle_total").html(crud.table.column(3).data().sum());
              $("#interest_total").html(crud.table.column(4).data().sum());
              $("#other_payment_total").html(crud.table.column(5).data().sum());
              $("#penalty_amount_total").html(crud.table.column(6).data().sum());
              $("#total_payment_total").html(crud.table.column(7).data().sum());
            });
    });
    ...
    $( "#excel" ).click(function() {
        ...
        var url = "{{URL::to('/admin/report/officer-collection/excel')}}?" + $.param(query)
        window.location = url;
    });
    </script>
@endsection
