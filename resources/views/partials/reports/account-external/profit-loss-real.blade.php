        ...
        <tbody>
            ...
            @if(isset($bals[40]))
                ...
                @forelse($bals[40] as $acc_id => $bal)
                    @php
                        $total_income += ($bal??0);
                         $external_account = \App\Models\AccountChartExternal::find($acc_id);
                    @endphp
                    <tr>
                        <td style="padding-left: 60px;">{{ optional($external_account)->external_acc_code."-".optional($external_account)->external_acc_name}}</td>
                    ...
                    @if(isset($bals[60]))
                        @if(count($bals[60])>0)
                            ...
                            @forelse($bals[60] as $acc_id => $bal)
                                ...
                                @endphp
                                <tr>
                                    <td style="padding-left: 60px;">{{ optional($external_account)->external_acc_code."-000".optional($external_account)->external_acc_name}}</td>
                                    <td style="text-align: right">{{ number_format($bal??0,2) }}</td>
                                </tr>
                            @endforeach
                            <tr>
