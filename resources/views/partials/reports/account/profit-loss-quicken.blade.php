...
<div id="DivIdToPrint">
    @if($bals != null)
        @include('partials.reports.header',['report_name'=>'Profit Loss','from_date'=>$start_date,'to_date'=>$end_date,'use_date'=>1])
            ...
            <tbody>
                                ...
                                @foreach($branches as $b_id)
                                    <?php
                                        if(!isset($total_cogs[$b_id])) $total_cogs[$b_id] = 0;
                                            ...
                                            $total = array_sum($bal);
                                    ?>
                                    <td style="text-align: right"><a style="text-decoration:underline;cursor: pointer" class="general-leg50" id="{{ $acc_id }}" data-branch="{{ $b_id }}">
                                        {{ RemoveDecimal(number_format($b??0,2)) }}</a>
                                    </td>
                                @endforeach
                                <td>{{RemoveDecimal(number_format($total,2))}}</td>
                            </tr>
                ...
                <tr>
                    ...
                    @foreach($branches as $b_id)
                        ...
                        ?>
                        @if ($n > 1)
                            @if($gross_profit[$b_id] <= 0)
                                <td style="color: green;text-align: right;"><u>{{ RemoveDecimal(number_format(-$gross_profit[$b_id],2)) }}</u></td>
                            @else
                        ...
                        @else
                            ...
                            @if($gross_profit[$b_id] < 0)
                                 <td style="color: green;text-align: right;padding-right: 20px"><u>{{ RemoveDecimal(number_format(-$gross_profit[$b_id],2))}}</u></td>
                            @else
                    ...
                    @if($n >1)
                        @if($total < 0)
                            <td>{{number_format(RemoveDecimal(-$total,2))}}</td>
                        @else
                                ...
                                @foreach($branches as $b_id)
                                    <?php
                                        if(!isset($total_expense[$b_id])) $total_expense[$b_id] = 0;
                                            ...
                                            $total = array_sum($bal);
                                    ?>
                                    <td style="text-align: right"><a style="text-decoration:underline;cursor: pointer"  class="general-leg60" id="{{ $acc_id }}" data-branch="{{ $b_id }}">{{ RemoveDecimal(number_format($b??0,2)) }}</a></td>
                                @endforeach
                                @if($total < 0)
                                    <td style="color:red;">{{RemoveDecimal(number_format($total,2))}}</td>
                                @else
                ...
                <tr>
                    @foreach($branches as $b_id)
                        ...
                        @endphp
                        @if ($n > 1)
                            @if($net_ordinary[$b_id] < 0)
                                <td style="color: blue;text-align: right;"><u>{{RemoveDecimal(number_format(-$net_ordinary[$b_id],2))}}</u></td>
                            @else
                        ...
                        @else
                            ...
                            @if($net_ordinary[$b_id] < 0)
                                <td style="color: blue;text-align: right;padding-right: 30px"><u>{{ RemoveDecimal(number_format(-$net_ordinary[$b_id],2)) }}</u></td>
                            @else
                                ...
                                @foreach($branches as $b_id)
                                    <?php
                                        if(!isset($total_other_expense[$b_id])) $total_other_expense[$b_id] = 0;
                                            ...
                                            $total = array_sum($bal);
                                    ?>
                                    <td style="text-align: right"><a style="text-decoration:underline;cursor: pointer" class="general-leg80" id="{{ $acc_id }}"  data-branch="{{ $b_id }}">{{ RemoveDecimal(number_format($b??0,2)) }}</a></td>
                                @endforeach
                                <td>{{RemoveDecimal(number_format($total,2))}}</td>
                            </tr>
                        @endforeach
                ...
                <tr>
                    ...
                    @foreach($branches as $b_id)
                        ...
                        @if($net_income[$b_id] != $net_ordinary[$b_id])
                            @if ($n > 1)
                                @if($net_income[$b_id] < 0)
                                    <td style="color: blue;text-align: right;"><u>{{ RemoveDecimal(number_format(-$net_income[$b_id],2)) }}</u></td>
                                @else
                            ...
                            @else
                                ...
                                @if($net_income[$b_id] < 0)
                                    <td style="color: blue;text-align: right;padding-right: 50px"><u>{{ RemoveDecimal(number_format(-$net_income[$b_id],2)) }}</u></td>
                                @else
...
<script>
    ...
    function getReport(id, branch_id){
        $("#whole_page_loader").fadeIn(function(){
            ...
            var url = "{{ url('report/general-leger')}}";
            $.ajax({
                url: url,
                type: 'GET',
                async: false,
