...
<input type="hidden" id="company" value="company.moeyan">
...
<div id="DivIdToPrint">
    ...
    <table border="1" class="table-data" id="table-data">
        <tbody>
            ...
            @php $total_current_asset = []; @endphp
            ...
            <tr>
                ...
                @if($total_current_asset[$br_id] < 0) 
                    <td style="text-align: right;padding-right: 10px;color:red;">
                        <u>{{ RemoveDecimal(number_format($total_current_asset[$br_id]??0,2)) }}</u>
                    </td>
                @endif
                @endforeach
                ...
                @if($total < 0) 
                    <td style="text-align: right;color:red;">
                        {{RemoveDecimal(number_format($total??0))}}
                    </td>
                @else
            ...
            <tr>
                <td colspan="{{count($branchs)+1}}" style="padding-left: 30px">
                    <b>Fixed Assets</b>
                </td>
            </tr>
            ...
            <tr>
                ...
                @if($total_fixed_asset[$br_id] < 0) 
                    <td style="text-align: right;padding-right: 10px;color:red;">
                        <u>{{ RemoveDecimal(number_format($total_fixed_asset[$br_id]??0,2)) }}</u>
                    </td>
                @endif
                @endforeach
            ...
            <tr>
                ...
                @foreach($branchs as $br_id)
                    ...
                    @if($total_asset < 0) 
                        <td style="color: blue;padding-right: 20px;text-align: right;color:red;">
                            <u>{{ RemoveDecimal(numb_format($total_asset,2)) }}</u>
                        </td>
                    @endif
                @endforeach
            ...
            <tr>
                <td colspan="{{count($branchs)+1}}" style="color: green;">
                    <b>Liabilities & Owners Equity</b>
                </td>
            </tr>
            <tr>
                <td colspan="{{count($branchs)+1}}" style="color: blue;padding-left: 10px">
                    <b>Liabilities</b>
                </td>
            </tr>
            ...
            <tr>
                ...
                @foreach($branchs as $br_id)
                    @php 
                    $net = $net_income[$br_id]??0;
                    $total += ($net_income[$br_id]??0);
                    @endphp
                    @if($net > 0)
                        <td>
                            <span style="float: right;color:red;">{{ RemoveDecimal(number_format(-$net,2)) }}</span>
                        </td>
                    @else
                        <td>
                            <span style="float: right">{{ RemoveDecimal(number_format(-$net,2)) }}</span>
                        </td>
                    @endif
                @endforeach
            ...
            <tr>
                <td style="color: green;">
                    <b>Total Liabilities & Owners Equity</b>
                </td>
                @php $total = 0; @endphp
                ...
                @foreach($branchs as $br_id)
                    @php 
                    $owner_total = ($total_all_owners_equity[$br_id]??0)+($total_all_liabilities[$br_id]??0);
                    $total += ($owner_total??0);
                    @endphp
                    @if($owner_total > 0)
                        <td style="color: red;padding-right: 20px;text-align: right;">
                            <u>{{ RemoveDecimal(number_format(-$owner_total??0,2)) }}</u>
                        </td>
                    @else
                        <td style="color: green;padding-right: 20px;text-align: right;">
                            <u>{{ RemoveDecimal(number_format(-$owner_total??0,2)) }}</u>
                        </td>
                    @endif
                @endforeach
