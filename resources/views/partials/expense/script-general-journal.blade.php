    ...
    <table class="table table-bordered table-striped table-condensed">
        <thead>
            <tr style="background-color:#3c8dbc;color:white;">
                ...
                <th class="text-center">{{_t('note')}}</th>
            ...
            @if($journal_details != null)
                @if(count($journal_details) > 0)
                    @foreach($journal_details as $journal_detail)
                        @include('partials.expense.add-detail',['rd' => $journal_detail])
                    @endforeach
                @endif
            @endif
        </tbody>
        <tfoot>
            <tr>
                <th class="text-right" colspan="2">{{_t('total')}}</th>
                <th class="text-right"><span class="total_dr">0</span><span class="currency"> </span></th>
                <th class="text-right" style="display: none;"><span class="total_cr">0</span><span class="currency"> </span></th>
                <th class="text-center"><i class="fa fa-trash-o"></i></th>
            </tr>
        </tfoot>
    </table>
    ...
    <script>
        ...
        $(function () {
           ...
           $('[name="acc_chart_id"]').on('change', function () {
              ...
              if (acc_chart_id > 0) {
                  $.ajax({
                      url:'{{url('admin/expense-add-detail')}}',
                      data:{
